...
{
    public partial class EditorFunct : Form
    {
        ...
        private void loadData()
        {
            if (cbxIdConst.SelectedValue is not null)
            {
                ...
                DataFunctions data = Traitements.GetFunctData(Value);

                tbxOrigin.TextAlign = HorizontalAlignment.Left;
                tbxOrigin.Font = new Font("Segoe UI", 9.0f);
                tbxOrigin.Text = data.FunctOrigin;
                tbxTransl.Text = data.FunctTransl;
                lblCategName.Text = "Functions";
        ...
        private void btnEmpty_Click(object sender, EventArgs e)
        {
            ...
            int IdNextTextTranslEmpty = Traitements.NextFunctionsTranslEmpty(Value);
        ...
        {
            List<CheckBoxOption> options = Traitements.GetListFunctionsID();
        ...
        private void btnSearch_Click(object sender, EventArgs e)
        {
            if (tbxSearch.Text.Length >= 3)
            {
                ...
                List<CheckBoxOption> options = Traitements.GetListFunctionIDByTerms(searchValue);
                if (options.Count > 0)
                {
                    cbxIdConst.DataSource = options;
                    cbxIdConst.DisplayMember = "Constant";  // Propriété à afficher dans le CheckBox
        ...
        */
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            if (cbxIdConst.SelectedValue is not null)
            {
                ...
                Traitements.SaveDataFunctions(NewTranslate, ID, Constant, TextOrigin, Checked);
            }
        ...
        private void btnNextCheck_Click(object sender, EventArgs e)
        {
            ...
            int IdNextTextTranslEmpty = Traitements.NextFunctionsNoCheck(Value);
            if (IdNextTextTranslEmpty > 1)
            {
                cbxIdConst.SelectedValue = IdNextTextTranslEmpty;
            }
